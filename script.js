...
function frameURL(i){ return `assets/frames/frame_${String(i).padStart(4,"0")}.webp`; }
...
function draw(index){
  const img = images[Math.max(0, Math.min(TOTAL-1, Math.round(index)))];
...
function scrollMap(){
  ...
  const end=start+hero.offsetHeight-innerHeight;
  const p=Math.max(0,Math.min(1,(scrollY-start)/Math.max(1,end-start)));
  target=p*(TOTAL-1);
}
addEventListener("scroll",scrollMap,{passive:true});
  ...
  function next(){
    for(let k=0;k<concurrency;k++){
      ...
      img.onload=img.onerror=()=>{
        ...
        if(loaded===TOTAL){
          images[0] && draw(0);
          setTimeout(()=>{preloader.style.opacity="0";preloader.style.visibility="hidden";},350);
        }
        next();
      };
      img.src=frameURL(i);
